...
odoo.define('website_slider.front_js', function (require) {
    ...
    publicWidget.registry.js_slider_snippet = publicWidget.Widget.extend({
        ...
        build: function (debug) {
            ...
            var self = this;
            var product_ids = self.$target.attr("data-product_ids");
            var category_ids = self.$target.attr('data-category_ids')
            var discount_policy = self.$target.attr('data-discount_policy')
            var slider_style_template = self.$target.attr("data-style-template");
            var selected_ui_options = self.$target.attr("data-ui-option");
            var name = self.$target.attr("data-name");
            ...
            'slider_style_template':slider_style_template,
            'selected_ui_options':selected_ui_options,
            'limit':limit,
            'sort_by':sort_by,
            'filter_id':filter_id,
            ...
            ajax.jsonRpc('/slider/render', 'call',params).then(function (data) {
                ...
                if($(self.$target).find(".group_website_designer").length == 0){
					$(self.$target).removeAttr('data-product_ids data-category_ids data-discount_policy data-style-template data-ui-option data-name data-limit data-filter_id data-sort_by')
				}
            });
        },
